<script lang="ts">
   ...
   // Union types that include empty string for form state
   type InvestmentSizeOrEmpty = InvestmentSize | "";
   type RiskAppetiteOrEmpty = RiskAppetite | "";
   type AvailabilityOrEmpty = Availability | "";

   // Type guard functions to check if values are valid (not empty strings)
   function isValidInvestmentSize(
      value: InvestmentSizeOrEmpty,
   ): value is InvestmentSize {
      return value !== "";
   }

   function isValidRiskAppetite(
      value: RiskAppetiteOrEmpty,
   ): value is RiskAppetite {
      return value !== "";
   }

   function isValidAvailability(
      value: AvailabilityOrEmpty,
   ...
   // Update your state definitions to use empty strings instead of undefined for radio groups
   let investorData = $state<{
      investmentSize: InvestmentSizeOrEmpty;
      preferredStages: InvestmentStage[];
      investmentHistory: string;
      riskAppetite: RiskAppetiteOrEmpty;
      portfolioCompanies: number;
   }>({
      investmentSize: onboarding.formData.investorInfo?.investmentSize || "",
      preferredStages:
         onboarding.formData.investorInfo?.preferredStages ||
         ([] as InvestmentStage[]),
      investmentHistory:
         onboarding.formData.investorInfo?.investmentHistory || "",
      riskAppetite: onboarding.formData.investorInfo?.riskAppetite || "",
      portfolioCompanies:
   ...
   let supporterData = $state<{
      supportType: string[];
      availability: AvailabilityOrEmpty;
      hourlyRate: number | null;
      expertise: string;
   }>({
      supportType: onboarding.formData.supporterInfo?.supportType || [],
      availability: onboarding.formData.supporterInfo?.availability || "",
   ...
   $effect(() => {
      ...
      if (currentRole === "investor") {
         if (!isValidInvestmentSize(investorData.investmentSize)) {
         ...
         if (!isValidRiskAppetite(investorData.riskAppetite)) {
         ...
         if (!isValidAvailability(supporterData.availability)) {
      ...
      const timeoutId = setTimeout(async () => {
         try {
            if (currentRole === "investor") {
               ...
               await onboarding.updateRoleSpecificData(currentRole, cleanData);
            } else if (currentRole === "support") {
   ...
   function cleanRoleData(role: UserRole, data: any) {
      if (role === "investor") {
         return {
            ...data,
            investmentSize: isValidInvestmentSize(data.investmentSize)
               ? data.investmentSize
               : null,
            riskAppetite: isValidRiskAppetite(data.riskAppetite)
               ? data.riskAppetite
               : null,
         };
         ...
         return {
            ...data,
            availability: isValidAvailability(data.availability)
               ? data.availability
               : null,
         };
      ...
      <div class="space-y-6">
         ...
         <Card>
            <CardHeader>
               ...
               <p class="text-sm">
                  Which stages are you most interested in? (Select multiple)
               </p>
            </CardHeader>
            <CardContent class="space-y-4">
         ...
         <Card>
            <CardHeader>
               ...
               <p class="text-sm">
                  Tell us about your investment experience and track record
               </p>
            </CardHeader>
            <CardContent>
               <Textarea
            ...
            <CardContent class="space-y-4">
               <div class="grid gap-3">
                  {#each supportTypes as type}
                     ...
                     >
                        <Checkbox
                           checked={supporterData.supportType.includes(type)}
      ...
      <Card>
         <CardHeader>
            ...
            <p class="text-sm text-muted">
               Founder-specific questions will be available soon. For now, you
