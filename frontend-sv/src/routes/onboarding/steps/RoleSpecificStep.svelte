<script lang="ts">
   ...
   let investorData = $state({
      ...
      preferredStages: onboarding.formData.investorInfo?.preferredStages || [],
      investmentHistory:
   ...
   $effect(() => {
      const timeoutId = setTimeout(() => {
         if (currentRole === "investor") {
            onboarding.updateRoleSpecificData(currentRole, investorData);
         } else if (currentRole === "support") {
            onboarding.updateRoleSpecificData(currentRole, supporterData);
         } else if (currentRole === "founder") {
            onboarding.updateRoleSpecificData(currentRole, founderData);
         }
      }, 500);
               ...
               <div class="grid gap-3">
                  {#each investmentStages as stage}
                     <div
                        ...
                        onclick={() => toggleInvestmentStage(stage.value)}
                     >
                        <Checkbox
