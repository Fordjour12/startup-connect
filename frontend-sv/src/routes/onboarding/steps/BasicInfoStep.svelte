<script lang="ts">
         ...
         : // For other sub-steps, only validate URL fields if they have content
           !["linkedinUrl", "githubProfile", "portfolioWebsite"].some(
              (field) => {
                 ...
                 return value && value.trim() && !/^https?:\/\/.+/.test(value);
              },
           ),
   );
   ...
   $effect(() => {
      ...
      if (currentSubStep === 2) {
         const urlFields = ["linkedinUrl", "githubProfile", "portfolioWebsite"];
   ...
   function validateCurrentStep(): boolean {
      ...
      console.log("isCurrentStepValid value:", isCurrentStepValid);
...
<div class="space-y-8 animate-fadeInUp">
      ...
      >
         ...
         <CardContent class="space-y-6 p-6">
            <div class="grid md:grid-cols-2 gap-6">
               ...
               <!-- Employment Status -->
               <div class="space-y-2">
                  <Label for="employment" class="text-foreground font-body"
   ...
   >
      <p class="font-body">All information is optional except name and email</p>
      <p class="font-body">You can always update your profile later</p>
   </div>

   <!-- Debug Section (remove this in production) -->
   <div class="mt-8 p-4 bg-muted/20 rounded-lg border border-border">
      <h3 class="text-sm font-semibold text-foreground mb-2">Debug Info:</h3>
      <div class="text-xs text-muted-foreground space-y-1">
         <p>Current Sub-step: {currentSubStep}</p>
         <p>Total Sub-steps: {totalSubSteps}</p>
         <p>Is Current Step Valid: {isCurrentStepValid ? "Yes" : "No"}</p>
         <p>Name Length: {formData.name.length}</p>
         <p>Email Valid: {formData.email ? "Yes" : "No"}</p>
         <p>Errors Count: {Object.keys(errors).length}</p>
         <p>Current Step: {onboarding.currentStep}</p>
         <p>Can Go Forward: {onboarding.canGoForward ? "Yes" : "No"}</p>
      </div>
   </div>
</div>
