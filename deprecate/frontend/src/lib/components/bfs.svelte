<!-- well use  this after understading how to use itr -->
<script lang="ts">
    ...
    // Convert FileList to File[] if needed - only in browser
    let fileArray = $derived(() => {
    ...
    function processFiles(newFiles: FileList | File[]) {
        ...
        if (validFiles.length > 0) {
            ...
            if (multiple) {
                finalFiles = [...fileArray(), ...validFiles];
            } else {
        ...
        {:else}
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between pt-2 border-t">
                    <p class="text-xs text-muted-foreground">
                        {fileArray().length} file{fileArray().length !== 1
                            ? "s"
                            : ""} selected
                    </p>
