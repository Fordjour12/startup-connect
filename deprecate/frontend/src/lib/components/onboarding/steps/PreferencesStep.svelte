		...
		formData = {
			geographicPreference:
				(onboardingState.formData
					.geographicPreference as Preferences["geographicPreference"]) ||
				("local" as Preferences["geographicPreference"]),
			communicationStyle:
				(onboardingState.formData
					.communicationStyle as Preferences["communicationStyle"]) ||
				(["email"] as Preferences["communicationStyle"]),
			workingStyle:
				(onboardingState.formData
					.workingStyle as Preferences["workingStyle"]) ||
				("collaborative" as Preferences["workingStyle"]),
			diversityPreference:
				onboardingState.formData.diversityPreference ?? true,
			notificationFrequency:
				(onboardingState.formData
					.notificationFrequency as Preferences["notificationFrequency"]) ||
				("weekly" as Preferences["notificationFrequency"]),
		};
	});
		...
		if (formData.communicationStyle.includes(style)) {
			if (formData.communicationStyle.length > 1) {
				formData.communicationStyle =
					formData.communicationStyle.filter((s) => s !== style);
			}
		} else {
			formData.communicationStyle = [
				...formData.communicationStyle,
			...
			<div class="flex items-start space-x-3">
				...
				<div class="space-y-2">
					...
					<ul class="text-blue-700 space-y-1 text-sm">
						<li>
							• Be specific about your communication preferences
							to get better matches
						</li>
						<li>
							• Your working style helps set expectations with
							potential collaborators
						</li>
						<li>
							• You can update these preferences anytime in your
							profile settings
						</li>
						<li>
